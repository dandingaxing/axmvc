...
class page {
    ...
    public $offset = 0;                    // 根据当前页数, 分页查询, offset 计算
    ...
    public function setUrlModel($urlmodel){
        $this->urlmodel = urldecode( $urlmodel );
        return $this;
    }
    ...
    public function getUrlModel(){
        ...
        $_getParams[$this->pageParam] = $this->pagepre;
        $this->urlmodel = urldecode( base::app('url')->admin($args['r'], $_getParams) );
        return $this->urlmodel;
    }

    public function setFirstModel($firstmodel){
        $this->firstmodel = urldecode( $firstmodel );
        return $this;
    }
    ...
    public function page( $ulclass="pagination pagination", $nowclass="active" ){
        ...
        if (empty($pageArr)) {
            return '<ul class="'.$ulclass.'">' . '<li class="disabled"><a href="">共 '.$this->total.' 条 / '.$this->pagecount.'页 </a></li>'. '</ul>';
        }
        // 首页
